...
pub fn init() {
    ...
    if log_path.exists() {
        let time: DateTime<Local> = log_path.metadata().expect("Could not read metatata of last log file!").modified().unwrap().into();
        ...
        last_file.write(&std::fs::read(log_path).unwrap()).expect("Could not copy to log file! ");
